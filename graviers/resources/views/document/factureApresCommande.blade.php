...
@section('mode_paiement', $paiement->lignePaiements->first()?->modePaiement?->libelle ?? 'N/A')
            ...
            @php
                ...
                switch($paiement->service) {
                    case 'COMMANDE':
                        ...
                        $serviceNumero = $paiement->commande?->numero ?? 'N/A';
                        break;
                    case 'LOCATION':
                        $serviceLabel = 'Paiement location';
                        $serviceNumero = $paiement->location?->numero ?? 'N/A';
                        break;
                    case 'LIVRAISON':
                        $serviceLabel = 'Paiement livraison';
                        $serviceNumero = $paiement->livraison?->numero ?? 'N/A';
                        break;
                    default:
                        $serviceLabel = 'Paiement';
                        $serviceNumero = $paiement->code;
                }
            @endphp
            <tr>
                <td class="col-ref">{{ $paiement->code }}</td>
                <td class="col-designation">{{ $serviceLabel }} n°{{ $serviceNumero }}</td>
                <td class="col-pu">{{ number_format($paiement->montant_total, 0, '', ' ') }}</td>
                <td class="col-qte">1</td>
                <td class="col-unite">Forfait</td>
                <td class="col-taxes">0</td>
                <td class="col-rem">0</td>
                <td class="col-montant">{{ number_format($paiement->montant_total, 0, '', ' ') }}</td>
            </tr>
        </tbody>
    </table>
@endsection
...
@section('totaux')
    <table class="fne-totaux-outer"><tr><td class="fne-totaux-spacer"></td><td class="fne-totaux-content"><table class="fne-totaux">
        <tr>
            ...
            <td class="valeur">{{ number_format($paiement->montant_total, 0, '', ' ') }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="valeur" style="font-size:10pt; font-weight:bold;">{{ number_format($paiement->montant_total, 0, '', ' ') }}</td>
        </tr>
    </table></td></tr></table>
@endsection
    ...
    <table class="fne-resume">
        ...
        <tbody>
            <tr>
                ...
                <td class="text-right">{{ number_format($paiement->montant_total, 0, '', ' ') }}</td>
