...
@php
    ...
    // ne peut pas être le 1er. On expose juste l'id de l'admin courant.
    $currentUserId = Auth::id();
@endphp

@extends('layout.main')
@section('title','Liste des demandes de paiements - livreurs')
...
@endif
                ...
                <div class="content-header">
                    <div>
                        <h2 class="content-title card-title"> Etat des paiements - livreurs</h2>
                        ...
                        <div class="table-responsive">
                            <table id="liste" class="table table-striped">
                                <thead>
                                        ...
                                        <th class="text-center" style="background-color: #1c57a3; color: white;">N° du livreur</th>
                                        <th class="text-center" style="background-color: #1c57a3; color: white;">N° pour le paiement</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($demandes as $demande)
                                        @if($demande->user->type_user_id == 8 && $demande->paye == false)
                                            @php
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    @elseif($estEnAttente1)
                                                        <a href="{{route('show.valideDemande',['id'=>$demande->id, 'type' => 'livreur','reponse' => 'accepter'])}}" class="btn btn-sm font-sm rounded btn-success"><i class="material-icons md-check"></i> 1re validation</a>
                                                    ...
                                                    @elseif($estEnAttente2)
                                                        <a href="{{route('show.valideDemande',['id'=>$demande->id, 'type' => 'livreur','reponse' => 'accepter'])}}" class="btn btn-sm font-sm rounded btn-success"><i class="material-icons md-check"></i> 2e validation (accepter)</a>
                                                    @endif
                                                </td>
                                                {{-- refuser --}}
                                                <td class="text-center">
                                                    @if($estDejaFinalisee)
                                                        —
                                                    @elseif($estEnAttente2 && !$estInitiateur)
                                                        <a href="{{route('show.valideDemande',['id'=>$demande->id, 'type' => 'livreur','reponse' => 'refuser'])}}" class="btn btn-sm font-sm rounded btn-danger" onclick="return confirm('Refuser cette demande ?');"><i class="material-icons md-denied"></i> Rejeter</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endif
